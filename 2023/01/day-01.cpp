...
void part1(std::vector<std::string> lines) {
  ...
  for (int i = 0; i < lines.size(); i++) {
    ...
    for (int left = 0, right = lines[i].size() - 1;
         left < lines[i].size() && right >= 0; left++, right--) {

      // if current character is a number and value isnt empty, assign that
      // character to value
  ...
  for (int i = 0; i < lines.size(); i++) {
    ...
    for (const auto &pair : numbers) {
      ...
      while ((start = lines[i].find(pair.first, start)) != std::string::npos) {
        ...
        // replace the substring from the first character to the last with its
        // corrisponding value
    ...
    // loop thorugh each string from ftb and btf
    for (int left = 0, right = lines[i].size() - 1; left < lines[i].size();
         left++, right--) {
...
int main() {
  ...
  // init vector to store input data
